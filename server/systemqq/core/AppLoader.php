...
class AppLoader{
	...
	public function model($path,array $args=[]){
		...
		if(file_exists($target))
				require_once $target;
		else if(strtolower(file_exists($target)))
	...
	public function library($path,$args=[]){
		...
		else if(file_exists(strtolower($sys_path)))
			irequire_once($sys_path);
		else if(file_exists(strtolower($app_path)))
	...
	*/
	...
	public function __call( $methodName, $arguments ) {
		if ( array_key_exists( $methodName, $this->classes ) ) {
			...
			else if(file_exists(strtolower($target)))
				return static::cache($target,irequire_once($target));
			else
				throw new Exception("File {$target}.php Not Found");
		}
		else 
